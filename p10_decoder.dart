...
library odw.sdk.base.io.p10_decoder;
...
//typedef Decoder([int limit]);

/// A DICOM model
/// The DataSet class encapsulates a collection of DICOM Elements and provides various functions
/// to access the type in those elements
class Decoder  extends DecoderBase implements Maps {
  final bool isExplicit;
  Decoder attributeReader;
  ByteArray buf;
  Map<int, Float> elements = {};
  ...
  Decoder.fromByteArray(this.buf, [bool this.isExplicit = true, Endianness endian]) {
    _init(buf.bytes, buf.start, buf.limit, isExplicit, endian);
  }

  Decoder.fromBuffer(ByteBuffer buffer, [int start = 0, int length, Endianness endian]) {
    _init(buffer.asUint8List(), start, length, isExplicit, endian);
  }

  Decoder.fromUint8List(Uint8List bytes, [int start = 0, int length, Endianness endian]) {
    _init(bytes, start, length, isExplicit, endian);
  }

  Decoder.view(Uint8List bytes, [int start = 0, int length, Endianness endian]) {
    _init(bytes, start, length, isExplicit, endian);
  }
  ...
  void _init(Uint8List bytes, int start, int length, bool isExplicit, Endianness endian) {
    ...
    attributeReader = (isExplicit) ? readElementExplicit : readElementImplicit;
  ...
  //****  Explicit VR Element Reader  ****
  //****  Decoder for Attributes with Explicit Value Representations  ****
  // Read an [Element] from the [ByteArray] containing a model with Explicit VRs.
  /// Read an [Element] with an *Explicit* VR from [buf] [ByteArray].
  Element readElementExplicit([int limit]) {
    ...
    if (vr == VR_SQ) {
      ...
      var element = new Float(tag, vr, offset, length);
      print('readElementExplicit: $element');
      return element;
    }
  }
  ...
  /// Read an [Element] with an *Implicit* VR from [buf].
  Float readElementImplicit([int limit]) {
    ...
    if (_peekTag() == kItem) {
      ...
      buf.seek(length);
      var element = new Element(tag, -1, offset, length);
      //print(element);
      return element;
    }
  }
  ...
  Sequence _readSequence(int tag, int offset, int length, int limit) {
  ...
  // DatasetExplicit & readElementExplicit
  Item _readItem() {
    ...
    while (buf.position < item.limit) {
      ...
      var element = attributeReader(item.limit);
      item.dataset[element.tag] = element;
      if (element.tag == kItemDelimitationItem) {
  ...
  readEncapsulatedPixelData(Dataset dataset, frame) {
    ...
    var pixelElement = dataset.attributes[0x7fe00010];
    if (pixelElement == null) {
      throw "readEncapsulatedPixelData: pixel type element x7fe00010 not present";
    }
  ...
  int numStringValues(tag) {
    ...
    var element = this.elements[tag];
    if ((element != null) && (element.length > 0)) {
      //print('NumStringValues: element=$element');
      var charCodes = buf.bytes.buffer.asUint8List(element.offset, element.length);
  ...
  string(int tag, [int index = 0]) {
    ...
    if (element != null) {
      ...
      var bytes = buf.bytes.buffer.asUint8List(element.offset, element.length);
      int char = bytes[bytes.length - 1];
      //print('string: char="$char"');
      if (char == $nul)
        bytes = buf.bytes.buffer.asUint8List(element.offset, element.length - 1);
  ...
  String text(tag, [index = 0]) {
    var element = this.elements[tag];
    if ((element != null) && (element.length != 0)) {
      var charCodes = new Uint8List.view(buf.bytes.buffer, element.offset, element.length);
  ...
  DateTime dateTime(tag) {
    ...
    if ((value != null) && (value.length == 8)) {
      ...
      return new DateTime(yyyy, MM, dd, hh, mm, ss, ffffff);
    }
  ...
  void format(Formatter fmt) {
    fmt.writeLn('Dataset:[');
    fmt.indent();
    elements.forEach((int tag, Element e) {
      e.format(fmt);
    });
    fmt.outdent();
    fmt.writeLn(']');
